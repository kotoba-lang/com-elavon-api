(ns elavon-api.main
  "Kotodama WASM entrypoint for the Elavon_api clean-room actor (L4) — Clojure port.
  ...
  facts (`elavon_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "elavon_api")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"elavon_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "elavon_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :elavon_api-compat/ready)
